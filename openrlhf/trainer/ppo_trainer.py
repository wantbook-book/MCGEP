...
class PPOTrainer(ABC):
    ...
    def fit(
        ...
        self.qa_dataloader = qa_dataloader
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            #     ):
            if isinstance(self.qa_dataloader.sampler, DistributedSampler):
                self.qa_dataloader.sampler.set_epoch(
                    episode, consumed_samples=0 if episode > start_episode else consumed_samples
                )
            pbar = tqdm(
                range(self.qa_dataloader.__len__()),
                desc=f"Episode [{episode + 1}/{args.num_episodes}]",
                disable=not self.strategy.is_rank_0(),
            )
            for rand_prompts, rand_solutions in self.qa_dataloader:
                for i, experience in enumerate(
                    self.experience_maker.make_experience_list(rand_prompts, rand_solutions, **self.generate_kwargs)
                ):
                    if i == 0:
