...
from openrlhf.datasets import PromptDataset, SFTDataset, PromptAndSolutionDataset
...
class ActorModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        # )
        self.num_update_steps_per_episodes = (
            len(self.qa_dataset) * args.n_samples_per_prompt // args.train_batch_size * args.max_epochs
        )
    ...
    def prepare_qa_datasets(self):
        ...
        prompts_data = prompts_data.select(range(min(args.max_samples, len(prompts_data))))
        self.qa_dataset = PromptAndSolutionDataset(
            prompts_data, self.tokenizer, strategy, input_template=args.input_template
        )
        self.prompts_dataloader = None
        self.qa_dataloader = strategy.setup_dataloader(
            self.qa_dataset, args.rollout_batch_size // strategy.world_size, True, True
        )
