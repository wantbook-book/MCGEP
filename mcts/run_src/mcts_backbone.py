...
class MCTS_Searcher:
    ...
    def do_rollout(self, root_node: MCTS_Node, rollout_id: int)->MCTS_Node:
        "Make the tree one layer better. (Train for one iteration.)"
        if self.mcts_use_prm:
            at_least_one_terminal = False
            while not at_least_one_terminal:
                verbose_print("==> Selecting a node...", self.verbose)
                path_1 = self._select_prm(root_node, rollout_id)
                leaf = path_1[-1]
                verbose_print(f"==> Expanding node {leaf.id}...", self.verbose)
                self._expand(leaf, rollout_id)
                # 这中间返回就不用simulate到底部了
                # if not self.mcts_use_prm:
                ...
                self._backpropagate_prm(path_1)
                max_value_child_idx = -1
                for child_i, child in enumerate(self.parent2children[leaf]):
                    if child.is_terminal():
                        if max_value_child_idx==-1 or child.node_value > self.parent2children[leaf][max_value_child_idx].node_value:
                            max_value_child_idx = child_i
                if max_value_child_idx != -1:
                    return self.parent2children[leaf][max_value_child_idx]
        else:
            verbose_print("==> Selecting a node...", self.verbose)
    ...
    def _select_prm(self, node: MCTS_Node, rollout_id: int) -> List[MCTS_Node]:
        "Find an unexplored descendent of `node`"
        path = []
        while True:
            path.append(node)
    ...
    def _backpropagate_prm(self, path: List[MCTS_Node]):
        leaf = path[-1]
        leaf_children = self.parent2children[leaf]
        for child in leaf_children:
            # 绝对是第一次
            self.N[child] = 1
            self.Q[child] = child.node_value

        for node in reversed(path):
            reward = sum([self.Q[child] for child in self.parent2children[node]]) / len(self.parent2children[node])
            self.Q[node] = reward
            self.N[node] += 1
            # explored 不会用到
            self.explored_nodes.add(node)
