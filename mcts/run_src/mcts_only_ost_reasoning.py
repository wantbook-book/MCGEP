...
import sys
# sys.path.append('.')
from mcts.models.IO_System import IO_System
from mcts.common.utils import read_txt, read_json
from mcts.eval_src.Evaluator import Evaluator, GSM8KEvaluator
from .mcts_backbone import MCTS_Searcher, MCTS_Node
from mcts.run_src.rstar_utils import (
    Node_Type,
    GeneratorError,
...
class OstOrAnswerGenerator:
    ...
    def generate_ost_step_or_final_answer(
        self,
        user_question: str,
        solution_trace: Dict[int, Dict[str, str]],
        paraphrased: bool,
        ...
        return ost_step_list, potential_answers_list, values_list

class OstOrAnswerNode(MCTS_Node):
    def __init__(
        self,
        parent: "OstOrAnswerNode",
        depth: int,
        node_type: Node_Type,
        verbose: bool = False,
        # --- For instantiating root node ---
        node_value: float = None,
        generator: OstOrAnswerGenerator = None,
        ...
        self.children: List["OstOrAnswerNode"] = []
        self.depth = depth
    ...
    def _create_children(self, last_step:bool=False):
        ...
        def do_action_generate_ost_or_final_answer(parent_is_subquestion=False, last_step=False):
            ...
            for ost_step, potential_answers, value in zip(ost_step_list, potential_answers_list, values_list):
                self.children.append(
                    OstOrAnswerNode(
                        parent=self,
                        depth=self.depth + 1,
    ...
    def is_valid_leaf_node(self):
        ...
        ) or self.node_type is Node_Type.DIRECT_ANSWER or (
    ...
    def find_children(self, rollout_id: int):
        self.children = self.children or self._create_children(last_step=self.depth+1==self.max_depth_allowed)
    ...
    def calculate_reward(self):
        ...
        # ---jia
        elif self.node_type == Node_Type.OST_STEP and self.node_value is not None:
            return self.node_value
        # ---jia   
        else:
            return 0
...
def search_for_answers(args, user_question: str, question_id: int, gt_answer: str, generator: OstOrAnswerGenerator):
    verbose_print(
